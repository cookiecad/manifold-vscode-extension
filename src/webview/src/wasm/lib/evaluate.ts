...
export async function initManifold() {
  if(initialized) return manifoldwasm;

  if (!wasmUrl) throw new Error('WASM URL not set!');
...
export const getManifoldModule = () => {
  if(!initialized) throw new Error('Manifold not initialized');
  return manifoldwasm;
}
...
export class Evaluator {
   ...
   */
  constructor(module: ManifoldToplevel = manifoldwasm) {
    this.module = module;
